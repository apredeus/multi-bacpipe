...
NCDS=`grep -c "CDS$" $REF`
CHECK=`grep ">" $REF | grep -v "ncRNA$" | grep -v "misc$" | grep -v "CDS$"`

if [[ $CHECK != "" ]]
then
  echo "ERROR: extra reference contains features of type other than ncRNA/CDS/misc!"
  exit 1
fi
...
then
  ...
  echo
  echo "Validating extra CDS features by blasting external reference to all study strain genomes.."
  echo -e "\n--------------------------------------------------------------\n" 
  ...
  ## if only ncRNA/misc features, just use the reference as is
  echo "No coding sequences found in extra reference; using the sequences as is.."
  cp $REF $NEWREF
  echo -e "\n--------------------------------------------------------------\n" 
fi
