...
blastn -query $REF -db All_study_genomes.blast -evalue 1 -task megablast -outfmt 6 > All_study_genomes.ref_blast.out

## filter blast hits, convert them to GFF/BED, extract nucleotide sequences and convert to proteins
$SDIR/script/blast_to_gff.pl All_study_genomes.ref_blast.out $REF
grep -P "\tCDS\t" All_study_genomes.blast.gff | tr '=' '\t' | tr ';' '\t' | awk '{print $1"\t"$4-1"\t"$5"\t"$10"\t.\t"$7}' > All_study_genomes.CDS.bed
## this is here because bedtools 2.26 and 2.27 produce two different outputs with -name - this makes it version-independent 
